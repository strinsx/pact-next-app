    ...
    <div className="flex w-full flex-col rounded-2xl border-1 border-border bg-surface p-5">
      ...
      <div className="feed-scroll mt-4 max-h-[280px] flex-1 flex-col gap-3 overflow-y-auto pr-1">
        {loading && posts.length === 0 ? (
          <p className="py-10 text-center font-dm-sans text-sm text-muted">
            Loading...
          </p>
        ) : posts.length === 0 ? (
          <p className="py-10 text-center font-dm-sans text-sm text-muted">
          ...
          posts.map((post) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-r from-sky-400 to-purple font-dm-sans text-xs font-bold text-white">
                  {post.name[0].toUpperCase()}
                </div>
                <div className="flex flex-col">
                  <span className="font-dm-sans text-xs font-semibold text-primary">
                    {post.group}
                  </span>
                  <span className="flex items-center gap-1 font-dm-sans text-[11px] text-muted">
                    <Crown className="h-3 w-3 text-secondary" />
                    {post.name} · {post.time}
                  </span>
                </div>
              </div>
              <p className="mt-2 font-dm-sans text-xs">
